...
certificate. the Entando Operator will load all of these files into a Java keystore that it then configures as the 
...
`ENTANDO_DEFAULT_ROUTING_SUFFIX` environment variable specified on the Entando Operator's deployment. This specific secret 
...
and security specialists to verify that these ClusterRoles and RoleBindings
fall within the constraints of your organization's security governance. None of the  ClusterRoles in question
grant the Entando Operator `escalate` privileges on roles or `bind` privileges on RoleBindings. This implies that
the Entando Operator will never be able to give privileges to any other actors such as serviceaccounts that it doesn't have
itself already. the Entando Operator also does not have the privileges to read secrets in any namespaces other than its own,
...
Before deploying the Entando Operator, please first deploy the secrets it may use:
